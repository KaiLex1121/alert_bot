...
class Reminder(Base):
    ...
    is_active: Mapped[bool] = mapped_column(default=True, index=True) # Активно ли напоминание
    job_id: Mapped[Optional[str]] = mapped_column(index=True) # ID задачи в APScheduler
    frequency_type: Mapped[FrequencyType] = mapped_column(Enum(FrequencyType, name="frequency_type_enum"))
    ...
    apscheduler_job_id: Mapped[str | None] = mapped_column(String(255), unique=True, index=True) # Связь с задачей APScheduler
